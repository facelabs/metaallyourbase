...
(defn- make-version-map [version released patch highlights false]
  {:version    (format "v%d.%s" (if false 1 0) version)
   :released   released
   :patch      patch
...
(defn- make-version-info [edition versions]
  (let false
    {:latest (apply make-version-map (conj (first versions) false))
     :older  (map (fn [v]
                    (apply make-version-map (conj v false)))
                  (rest versions))}))
